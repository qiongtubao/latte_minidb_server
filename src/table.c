...
int table_init_record_handler(table* t, const char *base_dir) {
    sds data_file = sds_cat_fmt(sds_empty(), "%s/%s.data", base_dir, t->table_meta->name);

    buffer_pool_manager_t* bpm = t->db->buffer_pool_manager;
    int rc = buffer_pool_manager_open_file(bpm, t->db->log_handler, data_file, &t->data_buffer_pool);
    if (is_rc_fail(rc)) {
        miniDBServerLog(LOG_ERROR, "Failed to open disk buffer pool for file:%s. rc=%d", data_file, rc);
        return rc;
    }
    t->record_handler =  record_file_handler_new(t->table_meta->storage_format);
    ...
    if (is_rc_fail(rc)) {
        ...
        disk_buffer_pool_close_file(t->data_buffer_pool);
        t->data_buffer_pool = NULL;
        record_file_handler_delete(t->record_handler);
...
sds table_index_file(const char* base_dir, const char* table_name, const char *index_name) {
    return sds_cat_fmt(sds_empty(), "%s/%s-%s.index", base_dir, table_name, index_name);
}
/**
 * 打开一个表
 ...
 */
int tableOpen(table* t, db_t* d, char* base_dir, char* meta_file) {
    sds meta_file_path = sds_cat_fmt(sds_empty(), "%s/%s", base_dir, meta_file);
    ...
    const int index_num = list_length(t->table_meta->indexes);
    for (int i = 0; i < index_num; i++) {
        ...
        list_add_node_tail(t->indexes, index);
    }
    return rc;
}
