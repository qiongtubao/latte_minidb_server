 ...
 */
typedef struct Trx { 
    int (*redo)(Trx* trx, db_t* d, logEntry* entry);
} Trx;
...
typedef struct trxKit {
    Trx* (*create_trx)(trxKit* kit, log_handler_t* hanadler, int32_t trx_id);
    logReplayer* (*create_log_replayer)(trxKit* kit, db_t* d, log_handler_t* handler);
